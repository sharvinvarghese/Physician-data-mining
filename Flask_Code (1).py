...
def dat():
    if req.method == 'POST':
        ...
        df['new']=df[col1]+" "+ df[col2]+" "+df[col3]+" "+df[col4]+" "+df[phone]
        df['newcol']=df[col1]+" "+ df[col2]+" "+df[col3]+df[col4]+" "+" PHONE NUMBER "
        ...
        for i in df['new']:
            ...
            r = requests.get('https://google.com/search?q='+ i, verify = False) 
            ...
            if(ans==True):
                ...
                zi=df[phone][x-1]
                try:
                    f = requests.get(aurl, timeout=8, verify = False)
                    print(aurl)
                    z = f.text
                    zi = str(zi)
                    print(zi)
                ...
                resy = requests.get(url, verify = False)
                print(df['newcol'][x-1])
                small=df['newcol'][x-1]
                small=small.lower()
        ...
        df.to_csv('true.csv', mode='a', index = False, header=None)
...
def download_file():
    ...
    os.rename(r'outputdfi.csv',csv_file)
    path = csv_file
